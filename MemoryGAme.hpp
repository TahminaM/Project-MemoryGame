...
Description:
A: Download MemoryGame.hpp and MemoryGameClient.cpp
B: Download MemoryGame_partial.cpp and rename it as MemoryGame.cpp. Finish TODO Parts
-Implement swap and randomize functions, which will be used by MemoryGame class but are not member functions of MemoryGame
-Implement the following member functions and constructor / destructor in MemoryGame.cpp: default constructor, destructor, display, and play
C: Upload MemoryGame.hpp and MemoryGame.cpp to Gradescope for grading. (Client does not need to be uploaded to GS but is needed to run code locally)
Project 2 Files
*/
